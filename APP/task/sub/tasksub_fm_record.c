...
{
#if IIS_EN
    IIS_OUT_DISEN();    // 设置IIS DIN，消除录音后的噪音
#endif
    adc_stop();
    fm_sync_file();
    t_fm_rec.pos = 0;
...
void tasksub_fm_record_enter(void)
{
    ...
    mp3en_load_code(0x80);
#endif
    pll2_record_set(FM_CHANNEL_SEL);      // 录音时使用PLL2做IIS时钟
#else
    if (t_fm.fm_sta != FM_REC_PAUSE) {
        task_fm_record_init();
    }
    ...
    if(res == 2) {
        ...
        task_mic_display();         // 先显示1次，不然要录音1s后才有显示
#endif
    }
#if IIS_EN
    IE1 |= BIT(6);
    IIS_CON3 |= BIT(3); // KICK IIS Interrupt
#endif
}
...
#if IIS_EN
    amux_exit();        // 退出fm直通
    pll2_record_exit(); // 重新使用PLL1做IIS时钟
    work_sta_interrupt = TASK_FM;
    task_aux_eq_enter();
    IIS_OUT_EN();       // 重新设置IIS OUT
#endif
}
