...
#pragma location="TASK_MUSIC_SEG_1"
void music_file_num_set(bool Up_Down)
{
    sys_ctl.file_change_flag = 1;
    if(Up_Down)
    {
        task_music_next_dir();
    }
...
void task_music_event(void)
{
    ...
    if (device_change()) {
        if (!music_remount_device()) {
            if (!music_next_device()) {
                task_ctl.work_sta = TASK_AUX;  //设备无效，退出
                return;
            }
        }
        task_music_get_num();
    ...
    if (t_msc.file_change) {
        ...
        if(!sys_ctl.file_change_flag)show_num(t_msc.music_num);         //LCD段码屏及LED屏显示歌曲序号        
#endif
#if NPCP215X
    ...
    //判断音乐的播放状诚
    if ((music_get_status() <= STATUS_PLAY_STOPPING)&&(!sys_ctl.mp3_stop_flag)) {
