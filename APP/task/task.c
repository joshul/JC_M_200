...
void task_clear(void)
{
    ...
    user_set_volex(0);                  //默认需要3DB数字音量补偿
    ocx_init(NULL, NULL);               //清空默认的控件事件及消息
    ocx_clear();
    sys_ctl.sleep_cnt = 0xffff;
    sys_ctl.disp_bat_val = 0;
    sys_ctl.mp3_stop_flag = 0;
    sys_ctl.mute_flag = 0;//解静音标志
...
//    task_ctl.last_work = TASK_OPTI;     //上一次任务编号
    //task_ctl.work_sta = TASK_POWEROFF;
//    task_ctl.work_sta = TASK_OPTI;
...
void run_task(void)
{
    ...
    if((task_ctl.work_sta < TASK_AUX)||(task_ctl.work_sta > TASK_BT))task_ctl.work_sta = TASK_AUX;        
#endif    
    comm_event();
    while (1) {
        task_clear();       
        //if(task_ctl.work_sta != TASK_POWEROFF)sys_param_write8(PARAM_WORD_STA, task_ctl.work_sta);
        sys_param_write8(PARAM_WORD_STA, task_ctl.work_sta);
        ...
        case TASK_COAX:
            task_ctl.last_work = TASK_COAX;
            task_record();
            break;  
            
        case TASK_OPTI:
            task_ctl.last_work = TASK_OPTI;
            task_record();
            break;               
#endif
