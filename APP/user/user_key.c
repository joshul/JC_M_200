...
__near_func void key_scan(void)
{
    ...
    if(key_val == NO_KEY) {
        ...
        if(key_val!=NO_KEY)sys_ctl.irkey = 1;//ir标志置1
    }
#elif (SOFT_IR_EN || SOFT_IR_PORT_INT_EN)
    if(key_val == NO_KEY) {
        if(32 == ir.state){
...
#define STOP_CHARGE_TIME        20  //20s
#define START_CHAGRE_TIME       10  //10s
