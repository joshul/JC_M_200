...
{
	60-60,60-50,60-45,60-40,60-36,60-32,60-30,60-28,60-26,60-24,60-22,
	60-20,60-19,60-18,60-17,60-16,60-15,60-14,60-13,60-12,60-11,  
	60-10, 60- 9, 60- 8, 60-  7, 60- 6, 60- 5, 60- 4,  60-  3, 60- 2, 60- 1,  
	60, 60+1, 
};*/
u8 const tbl_volume[VOLUME_MAX+1] =
{
    60-60,60-50,60-45,60-40,60-36,60-32,60-30,60-28,60-26,60-24,
    60-22,60-21,60-20,60-19,60-18,60-17,60-16,60-15,60-14,60-13,
    60-12,60-11,60-10,60- 9,60- 8,60- 7,60- 6,60- 5,60- 4,60- 3, 
    60- 2,60- 1,60, 
};
...
void user_setting_factory_sys(u8 *setting_buf)
{
    ...
    setting_buf[PARAM_WORD_STA] = TASK_BT;
    
    sys_ctl.main_vol = 16;
    setting_buf[PARAM_MAIN] = 16;
    
    sys_ctl.Trable_vol = 0;
    setting_buf[PARAM_TREBLE]  = 0;       //高音

    sys_ctl.Bass_vol = 0;
    setting_buf[PARAM_BASS] = 0;             //低音调
    
    sys_ctl.Sub_vol = 16;
    setting_buf[PARAM_SUB] = 16;
    
    sys_ctl.m3d_Mode = 0;
    setting_buf[PARAM_3D]= 0;                  //3d
    
    sys_ctl.dim_level = 2;
    setting_buf[PARAM_DIM] = 2;             //dim 
    
    sys_ctl.Eq_tyle = 0;
    setting_buf[PARAM_EQ] = 0;
   ...
   // setting_buf[PARAM_MODE_RECORD] = 1;
    ...
    sys_ctl.volume = VOLUME_MAX;                //音量
    //setting_buf[PARAM_VOLUME] = VOLUME_MAX;
...
{ 
#if 0
    task_ctl.work_sta = sys_param_read8(PARAM_WORD_STA);
    if(task_ctl.work_sta > TASK_BT){
        return false;
    }   
#endif 
    sys_ctl.main_vol = setting_buf[PARAM_MAIN];             //音量
    if ((sys_ctl.main_vol < 1)||(sys_ctl.main_vol > 33))
    {
		sys_ctl.main_vol = 16;
    }else{
		sys_ctl.main_vol -= 1;
    ...
    sys_ctl.Trable_vol = setting_buf[PARAM_TREBLE];         //高音
    if ((sys_ctl.Trable_vol < 1)||(sys_ctl.Trable_vol > 21))
    {
		sys_ctl.Trable_vol = 0;
    }else{
		sys_ctl.Trable_vol -= 11;
	}

    sys_ctl.Bass_vol = setting_buf[PARAM_BASS];             //低音调
    if ((sys_ctl.Bass_vol < 1)||(sys_ctl.Bass_vol > 21))
    {
		sys_ctl.Bass_vol = 6;
    }else{
		sys_ctl.Bass_vol -= 11;
    ...
    sys_ctl.Sub_vol = setting_buf[PARAM_SUB];               //低音
    if ((sys_ctl.Sub_vol < 1)||(sys_ctl.Sub_vol > 33))
    {
		sys_ctl.Sub_vol = 16;
    }else{
		sys_ctl.Sub_vol -= 1;
    ...
    sys_ctl.dim_level = setting_buf[PARAM_DIM];
    if ((sys_ctl.dim_level < 1)||(sys_ctl.dim_level > 4))
    {
		sys_ctl.dim_level = 2;
    }else{
		sys_ctl.dim_level -= 1;
    ...
    sys_ctl.Eq_tyle = setting_buf[PARAM_EQ];           //语言
    if (( sys_ctl.Eq_tyle < 1)||( sys_ctl.Eq_tyle > 3))
    {
		 sys_ctl.Eq_tyle = 0;
    }else{
		 sys_ctl.Eq_tyle -= 1;
    ...
    sys_ctl.m3d_Mode = setting_buf[PARAM_3D];                  //3d
    ...
    sys_ctl.eq_num = 0;//setting_buf[PARAM_EQ_NUM];             //EQ选择
   ...
   // sys_ctl.mode_record = setting_buf[PARAM_MODE_RECORD];   //录音
    ...
    sys_ctl.volume =VOLUME_MAX;//setting_buf[PARAM_VOLUME];             //音量
...
bool user_change_volume(u8 volume)
{
    ...
    if (volume <= VOLUME_MAX) {
#if IIS_EN
        dac_vol = tbl_volume[VOLUME_MAX] + sys_ctl.vol_ex;
#else
...
{
    if (volume <= VOLUME_MAX) {
        u16 dac_vol = 0xff00;//digital_volume[volume];
