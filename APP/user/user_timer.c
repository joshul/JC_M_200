...
__near_func void timer1s_hook(void)
{
    ...
    sys_ctl.bonoff_1s = !sys_ctl.bonoff_1s;
    put_msg(QSYSTEM_1S);
...
__near_func void timer100ms_hook(void)
{
    ...
    if (timer100ms_cnt % 5 == 0) {
        sys_ctl.bonoff = !sys_ctl.bonoff;
        flicker_500ms_cnt++;
        put_msg(QSYSTEM_500MS);
    }
#endif
...
#if (EAR_DETECT_EN && !EAR_SEG_REUSE && !EAR_ADKEY_REUSE)
     earphone_detect();
#endif
