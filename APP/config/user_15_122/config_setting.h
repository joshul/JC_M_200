...
#define THEME_SELECT                    THEME_LEDSEG_7PIN//THEME_LEDSEG_5C7S//THEME_NO//
...
#define BT_AUTO_STANDBY_EN              0   //BT任务下，是否需要自动进入休眠待机
...
#define WARNING_VOLUME                  16  //提示音的音量

#define WARNING_POWER_ON                0   //开机提示音
#define WARNING_TASK_BT                 0   //蓝牙任务提示音
#define WARNING_TASK_CLOCK              0   //时钟任务提示音
#define WARNING_TASK_AUX                0   //AUX/AUX EQ任务提示音
#define WARNING_TASK_FM                 1   //FM任务提示音
#define WARNING_TASK_VOICE              0   //录音播放任务提示音
#define WARNING_TASK_SPEAKER            0   //扩音器任务提示音
#define WARNING_TASK_MIC                0   //MIC录音任务提示音
#define WARNING_TASK_USBDEV             0   //PC任务提示音
...
#define WARNING_VOL_MAXMIN              0   //是否需要音量调节到最大或最小时提示音
#define WARNING_USB_SD                  0   //播放U盘或SD卡提示音
 ...
 *****************************************************************************/
//FM类型选择
#define FM_TYPE                         FM_RDA5807//FM_RTC6218
...
#define BT_CALL_PRIVATE                 1     //是否强制使用私密接听（手机端接听）
#define BT_RING_NUMBER                  0     //是否需要来电报号
#define BT_NAME_WITH_ADDR               0     //蓝牙名称是否显示3位地址（调试用，例如：BW-BTBOX-***）
#define BT_SAMPLERATE_16K               1     //蓝牙通话是否使用16k采样，0为8k采样
#define BT_HSHF_EN                      0     //是否打开通话HSHF服务
...
#define BT_3WCALL_EN                    0     //是否支持三方通话
...
#define WKO_WKUP_POWERDOWN              0       //关机是否使用WKO唤醒
...
#define USE_P35_USBPOW_CTL              2       //P35脚的使用选择。用做MUTE脚时为0，用做控制USB供电时选1，其他用处时为2
...
#define USE_IOKEY                       0           //ADKey的使用，0为不使用，1为使用IOKey
...
#define HARD_IR_EN                      1           //红外遥控（硬解，只有外部接32K晶振，才能使用硬解，其它请使用软解）
#define SOFT_IR_EN                      0           //红外遥控（NEC软解）
#define SOFT_IR_PORT_INT_EN             0
#define IR_PORT_INT_SEL			IR_PORT_INT_P07
...
#define OTHER_ADC_CHECK()

/*****************************************************************************
 * Module    : IIC的相关配置
 *****************************************************************************/
#define IIC_IO_SET
#define IIC_DATA_PORT                   3       //P32
#define IIC_DATA                        2
#define IIC_CLK_PORT                    3       //P31
#define IIC_CLK                         1

#define __IIC_DATA_CLK_OUT(DATA_PORT, CLK_PORT)     P ## DATA_PORT ## PU &= ~BIT(IIC_DATA);\
                                                    P ## CLK_PORT ## PU &= ~BIT(IIC_CLK);\
                                                    P ## DATA_PORT ## DIR &= ~BIT(IIC_DATA);\
                                                    P ## CLK_PORT ## DIR &= ~BIT(IIC_CLK);\
                                                    delay(10)                                       // 第二中间转换宏
#define _IIC_DATA_CLK_OUT(DATA_PORT, CLK_PORT)      __IIC_DATA_CLK_OUT(DATA_PORT, CLK_PORT)         // 第一中间转换宏
#define IIC_DATA_CLK_OUT()                          do{\
                                                        _IIC_DATA_CLK_OUT(IIC_DATA_PORT, IIC_CLK_PORT);\
                                                    }while(0)
#define __IIC_DATA_IN(PORT)                         P ## PORT ## DIR |= BIT(IIC_DATA);\
                                                    P ## PORT ## PU |= BIT(IIC_DATA)                // 第二中间转换宏
#define _IIC_DATA_IN(PORT)                          __IIC_DATA_IN(PORT)                             // 第一中间转换宏
#define IIC_DATA_IN()                               do{\
                                                        _IIC_DATA_IN(IIC_DATA_PORT);\
                                                    }while(0)
#define __IIC_DATA_OUT(PORT)                        P ## PORT ## PU |= BIT(IIC_DATA);\
                                                    P ## PORT ## DIR &= ~BIT(IIC_DATA)              // 第二中间转换宏
#define _IIC_DATA_OUT(PORT)                         __IIC_DATA_OUT(PORT)                            // 第一中间转换宏
#define IIC_DATA_OUT()                              do{\
                                                        _IIC_DATA_OUT(IIC_DATA_PORT);\
                                                    }while(0)
#define __IIC_DATA_CLK_HIGH(DATA_PORT, CLK_PORT)    do{\
                                                        P ## CLK_PORT |= BIT(IIC_CLK);\
                                                        P ## DATA_PORT |= BIT(IIC_DATA);\
                                                    }while(0)                                       // 第二中间转换宏
#define _IIC_DATA_CLK_HIGH(DATA_PORT, CLK_PORT)     __IIC_DATA_CLK_HIGH(DATA_PORT, CLK_PORT)        // 第一中间转换宏
#define IIC_DATA_CLK_HIGH()                         _IIC_DATA_CLK_HIGH(IIC_DATA_PORT, IIC_CLK_PORT)

#define __IIC_DATA_HIGH(DATA_PORT)                  P ## DATA_PORT |= BIT(IIC_DATA)     // 第二中间转换宏
#define _IIC_DATA_HIGH(DATA_PORT)                   __IIC_DATA_HIGH(DATA_PORT)          // 第一中间转换宏
#define IIC_DATA_HIGH()                             _IIC_DATA_HIGH(IIC_DATA_PORT)

#define __IIC_DATA_LOW(DATA_PORT)                   P ## DATA_PORT &= ~BIT(IIC_DATA)    // 第二中间转换宏
#define _IIC_DATA_LOW(DATA_PORT)                    __IIC_DATA_LOW(DATA_PORT)           // 第一中间转换宏
#define IIC_DATA_LOW()                              _IIC_DATA_LOW(IIC_DATA_PORT)

#define __IIC_CLK_HIGH(CLK_PORT)                    P ## CLK_PORT |= BIT(IIC_CLK)       // 第二中间转换宏
#define _IIC_CLK_HIGH(CLK_PORT)                     __IIC_CLK_HIGH(CLK_PORT)            // 第一中间转换宏
#define IIC_CLK_HIGH(CLK_PORT)                      _IIC_CLK_HIGH(IIC_CLK_PORT)

#define __IIC_CLK_LOW(CLK_PORT)                     P ## CLK_PORT &= ~BIT(IIC_CLK)      // 第二中间转换宏
#define _IIC_CLK_LOW(CLK_PORT)                      __IIC_CLK_LOW(CLK_PORT)             // 第一中间转换宏
#define IIC_CLK_LOW()                               _IIC_CLK_LOW(IIC_CLK_PORT)

#define __IIC_DATA_IS_HIGH(DATA_PORT)               (P ## DATA_PORT & BIT(IIC_DATA))    // 第二中间转换宏
#define _IIC_DATA_IS_HIGH(DATA_PORT)                __IIC_DATA_IS_HIGH(DATA_PORT)       // 第一中间转换宏
#define IIC_DATA_IS_HIGH()                          _IIC_DATA_IS_HIGH(IIC_DATA_PORT)

/*****************************************************************************
 * Module    : IIS的相关配置
 *****************************************************************************/
// 这里仅列举出通用的IIS配置。具体器件的配置需要到各自的头文件中配置
// 详细注意事项见user_iis.c
#define IIS_EN                          1               // 是否启用IIS功能
#define IIS_TYPE                        0//IIS_TAS5711     // IIS器件选择
...
#define LED_BLUE_IS_ON()                0//(P1PU & BIT(LED_BLUE))
...
//#define SPEAKER_MUTE()                  TAS5711_AMP_Mute_Set(TRUE);
//#define SPEAKER_UNMUTE()                TAS5711_AMP_Mute_Set(FALSE);
//#else
#define SPEAKER_MUTE()                  //P1DIR &= ~BIT(7); P1 |= BIT(7)
#define SPEAKER_UNMUTE()                //P1DIR &= ~BIT(7); P1 &= ~BIT(7)
//#endif
#define MUTE                            1
#define UNMUTE                          0
...
#define AUX_LINEIN()                    //P3DIR &= ~BIT(5); P3 &= ~BIT(5);P1DIR &= ~BIT(5); P1 &= ~BIT(5) //B:0   A:0
#define AUX_OPT()                       //P3DIR &= ~BIT(5); P3 |=BIT(5);P1DIR &= ~BIT(5); P1 |= BIT(5)    //B:1   A:1   //P3DIR &= ~BIT(5); P3 &= ~BIT(5);P1DIR &= ~BIT(5); P1 |=BIT(5)   //B:0   A:1
#define AUX_LIN()                       //P3DIR &= ~BIT(5); P3 |=BIT(5);P1DIR &= ~BIT(5); P1 &= ~BIT(5)   //B:1   A:0
#define AUX_HD()                        //P3DIR &= ~BIT(5); P3 &= ~BIT(5);P1DIR &= ~BIT(5); P1 |=BIT(5)   //B:0   A:1 //P3DIR &= ~BIT(5); P3 |=BIT(5);P1DIR &= ~BIT(5); P1 |= BIT(5)  //B:1   A:1
...
#endif
#ifndef IS_LINEIN_IN
#define IS_LINEIN_IN()                  0//(!(P4 & BIT(0)))
#endif
#endif
...
#define NFC_DETECT_INIT()               P3DIR |= BIT(4);   //

#define IS_NFC_WAKEUP()                 (!(P3 & BIT(4)))  //ADkey 按下
