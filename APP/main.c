...
extern void ir_port_int_init(void);
#endif
...
void sys_init(void)
{
    ...
    //ledseg_disp(MENU_POWERON);   //开机显示
#if IIS_EN
    //delay_ms(200);
    delay_5ms(200);
    iis_init();
    //delay_5ms(100); // 利用I2S器件的不完全初始化跳过dac初始化时的啪声
#endif
//ledseg_disp(MENU_POWERON);   //开机显示
#if DAC_MIX_OUT_EN
    ...
    ledseg_disp(MENU_MODEL);   //利用iis初始化延时做显示
#if WARNING_POWER_ON
